...
def add_mask(model, init_mask_dict=None):
    ...
    if init_mask_dict is None:
        ...
        mask1_train = nn.Parameter(torch.ones_like(model.gc1.weight))
        mask1_fixed = nn.Parameter(torch.ones_like(model.gc1.weight), requires_grad=False)
        mask2_train = nn.Parameter(torch.ones_like(model.gc2.weight))
        mask2_fixed = nn.Parameter(torch.ones_like(model.gc2.weight), requires_grad=False)
    ...
    AddTrainableMask.apply(model.gc1, 'weight', mask1_train, mask1_fixed)
    AddTrainableMask.apply(model.gc2, 'weight', mask2_train, mask2_fixed)
 
        
def generate_mask(model):

    mask_dict = {}
    mask_dict['mask1'] = torch.zeros_like(model.gc1.weight)
    mask_dict['mask2'] = torch.zeros_like(model.gc2.weight)
...
def subgradient_update_mask(model, args):
    ...
    model.gc1.weight_mask_train.grad.data.add_(args.s2 * torch.sign(model.gc1.weight_mask_train.data))
    model.gc2.weight_mask_train.grad.data.add_(args.s2 * torch.sign(model.gc2.weight_mask_train.data))
...
def get_mask_distribution(model, if_numpy=True):
    ...
    weight_mask_tensor0 = model.gc1.weight_mask_train.flatten()    # 22928
    nonzero = torch.abs(weight_mask_tensor0) > 0
    weight_mask_tensor0 = weight_mask_tensor0[nonzero]

    weight_mask_tensor1 = model.gc2.weight_mask_train.flatten()    # 22928
...
def get_final_mask_epoch(model, adj_percent, wei_percent):
    ...
    #print('wei_thre_index', wei_thre_index)
    wei_thre = wei_y[wei_thre_index]

    mask_dict = {}
    ori_adj_mask = model.adj_mask1_train.detach().cpu()
    #print('ori_adj_mask', ori_adj_mask)
    # ori_adj_mask.add_((2 * torch.rand(ori_adj_mask.shape) - 1) * 1e-5)
    mask_dict['adj_mask'] = get_each_mask(ori_adj_mask, adj_thre)
    mask_dict['weight1_mask'] = get_each_mask(model.gc1.state_dict()['weight_mask_train'], wei_thre)
    mask_dict['weight2_mask'] = get_each_mask(model.gc2.state_dict()['weight_mask_train'], wei_thre)
    
    #print('after adj mask', mask_dict['adj_mask'])
    return mask_dict
...
def get_final_weight_mask_epoch(model, wei_percent):
    ...
    weight1 = model.gc1.weight_orig_weight.detach().cpu().flatten()
    weight2 = model.gc2.weight_orig_weight.detach().cpu().flatten()
    ...
    mask_dict = {}
    mask_dict['weight1_mask'] = get_each_mask(model.gc1.state_dict()['weight_orig_weight'], wei_thre)
    mask_dict['weight2_mask'] = get_each_mask(model.gc2.state_dict()['weight_orig_weight'], wei_thre)
...
def oneshot_weight_magnitude_pruning(model, wei_percent):
    ...
    pdb.set_trace()
    model.gc1.weight_mask_train.requires_grad = False
    model.gc2.weight_mask_train.requires_grad = False
    ...
    weight1_mask = get_each_mask(model.gc1.state_dict()['weight_mask_train'], wei_thre)
    weight2_mask = get_each_mask(model.gc2.state_dict()['weight_mask_train'], wei_thre)
...
def random_pruning(model, adj_percent, wei_percent):
    ...
    wei1_nonzero = model.gc1.weight_mask_train.nonzero()
    wei2_nonzero = model.gc2.weight_mask_train.nonzero()
    ...
    for i, j in wei1_pruned:
        model.gc1.weight_mask_train[i][j] = 0
        model.gc1.weight_mask_fixed[i][j] = 0
    
    for i, j in wei2_pruned:
        model.gc2.weight_mask_train[i][j] = 0
        model.gc2.weight_mask_fixed[i][j] = 0
    
    model.adj_mask1_train.requires_grad = True
    model.gc1.weight_mask_train.requires_grad = True
    model.gc2.weight_mask_train.requires_grad = True
...
def print_sparsity(model):
    ...
    weight1_total = model.gc1.weight_mask_fixed.numel()
    weight2_total = model.gc2.weight_mask_fixed.numel()
    weight_total = weight1_total + weight2_total

    weight1_nonzero = model.gc1.weight_mask_fixed.sum().item()
    weight2_nonzero = model.gc2.weight_mask_fixed.sum().item()
...
def add_trainable_mask_noise(model, c):
    ...
    rand2 = (2 * torch.rand(model.gc1.weight_mask_train.shape) - 1) * c
    rand2 = rand2.to(model.gc1.weight_mask_train.device)
    rand2 = rand2 * model.gc1.weight_mask_train
    model.gc1.weight_mask_train.add_(rand2)

    rand3 = (2 * torch.rand(model.gc2.weight_mask_train.shape) - 1) * c
    rand3 = rand3.to(model.gc2.weight_mask_train.device)
    rand3 = rand3 * model.gc2.weight_mask_train
    model.gc2.weight_mask_train.add_(rand3)
...
def soft_mask_init(model, init_type, seed):
    ...
    if init_type == 'all_one':
        ...
        model.adj_mask1_train.requires_grad = True
        init.kaiming_uniform_(model.gc1.weight_mask_train, a=math.sqrt(5))

        model.gc1.weight_mask_train.requires_grad = False
        model.gc1.weight_mask_train.mul_(model.gc1.weight_mask_fixed)
        model.gc1.weight_mask_train.requires_grad = True

        init.kaiming_uniform_(model.gc2.weight_mask_train, a=math.sqrt(5))

        model.gc2.weight_mask_train.requires_grad = False
        model.gc2.weight_mask_train.mul_(model.gc2.weight_mask_fixed)
        model.gc2.weight_mask_train.requires_grad = True
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.normal_(model.gc1.weight_mask_train, mean=mean, std=std)

        model.gc1.weight_mask_train.requires_grad = False
        model.gc1.weight_mask_train.mul_(model.gc1.weight_mask_fixed)
        model.gc1.weight_mask_train.requires_grad = True

        init.normal_(model.gc2.weight_mask_train, mean=mean, std=std)
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.uniform_(model.gc1.weight_mask_train, a=a, b=b)

        model.gc1.weight_mask_train.requires_grad = False
        model.gc1.weight_mask_train.mul_(model.gc1.weight_mask_fixed)
        model.gc1.weight_mask_train.requires_grad = True

        init.uniform_(model.gc2.weight_mask_train, a=a, b=b)
